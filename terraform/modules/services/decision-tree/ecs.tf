...
resource "aws_ecs_service" "decision_tree_db" {
  name             = "SCALE-EU2-${upper(var.environment)}-APP-ECS_Service_DecisionTreeDB"
  cluster          = var.ecs_cluster_id
  task_definition  = aws_ecs_task_definition.decision_tree.arn
  launch_type      = "FARGATE"
  platform_version = "LATEST"
    ...
    [
      {
        ...
        "logConfiguration": {
          "logDriver": "awslogs",
          "options": {
              "awslogs-group": "${aws_cloudwatch_log_group.fargate_scale_db.name}",
              "awslogs-region": "eu-west-2",
...
resource "aws_cloudwatch_log_group" "fargate_scale_db" {
